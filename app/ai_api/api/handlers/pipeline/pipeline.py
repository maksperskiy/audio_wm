...
class PipelineHandler:
    ...
    async def classify(cls, audio: np.ndarray, samplerate: int) -> str:
        ...
        result = requests.post(f"{settings.CLASSIFIER_URI}/v1/models/classifier:predict", data=json.dumps({"inputs": audio.tolist()}))
        if result:
            labels = {}
            with open(
                "/app/ai_api/api/handlers/pipeline/assets/yamnet_class_map.csv", newline=""
            ) as csvfile:
                reader = csv.DictReader(csvfile)
                for row in reader:
                    labels[int(row["index"])] = row["display_name"]

            return labels.get(np.array(json.loads(result.content)["outputs"]["output_0"]).argmax())
    ...
    async def separate(audio: np.ndarray) -> dict:
        ...
        return prediction['vocals'][:,0]*32768, prediction['accompaniment'][:,0]*32768
